package com.example.springbootenterprise.component;

import com.example.springbootenterprise.dao.SysResourceRepository;
...
public class FilterInvocationSecurityMetadataSourceImpl implements FilterInvocationSecurityMetadataSource {
    ...
    public Collection<ConfigAttribute> getAttributes(Object o) throws IllegalArgumentException {
        ...
        SysResource resource = sysResourceRepository.findByName(requestUrl);

        //如果没有匹配的url则说明大家都可以访问
        if(resource == null) {
            System.out.println("没有匹配URL，默认可以访问");
            return SecurityConfig.createList("ROLE_LOGIN");
        }

        //将resource所需要到的roles按框架要求封装返回（ResourceService里面的getRoles方法是基于RoleRepository实现的）
        List<SysRole> roles = resource.getRoles();
        int size = roles.size();
